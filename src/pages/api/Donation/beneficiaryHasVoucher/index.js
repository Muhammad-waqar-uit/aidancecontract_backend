...
export default async function handler(req, res) {
  ...
  try {
    const ifhaveVoucher = await DAOHandler.methods
      .beneficiaryHasVoucherInCampaign(address, campaignId)
      .call();
    return res.status(200).json({ HasVoucher: ifhaveVoucher.toString() });
  } catch (error) {
    console.error(error);
