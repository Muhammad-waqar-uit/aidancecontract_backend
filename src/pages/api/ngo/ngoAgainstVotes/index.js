...
export default async function handler(req, res) {
  ...
  if (!ngoNumber) {
    return res.status(400).json({ error: "Registration No of ngo is required." });
  }
  try {
    const totalAgainstVote = await DAOHandler.methods.againstVotes(ngoNumber).call();
    return res.status(200).json({ TotalAgainstVote: totalAgainstVote.toString() });
  } catch (error) {
    console.error(error);
